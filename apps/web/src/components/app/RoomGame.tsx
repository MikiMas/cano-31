...
import { FinalResultsScreen } from "@/components/app/FinalResultsScreen";
...
export function RoomGame({ roomCode }: { roomCode: string }) {
  ...
  const refreshChallenges = useCallback(async () => {
    ...
    const nextState = res.data.paused ? ((res.data as any).state ?? null) : "running";
    setState((prev) => {
      // Once we’ve started, avoid bouncing back to the lobby due to a transient "scheduled" response.
      if ((prev === "running" || prev === "paused") && nextState === "scheduled") return prev;
      return nextState;
    });
  ...
  const waitForGameToStart = useCallback(async () => {
    const startedAt = Date.now();
    while (Date.now() - startedAt < 12_000) {
      const data = await refreshChallenges();
      const maybeState = (data as any)?.state as string | undefined;
      if (data && (data as any)?.paused === false) return true;
      if (maybeState && maybeState !== "scheduled") return true;
  ...
  if (startingGame) return <LoadingScreen title="Empezando partida…" />;
      ...
      {!player ? (
        ...
        <>
            ...
            <section className="card">
              ...
              {isOwner ? (
                <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
                  ...
                  >
                    {startingGame ? "Empezando..." : "Empezar ahora"}
                  </button>
                  <button
                    onClick={() => {
          ...
          {state !== "scheduled" && state === "ended" ? (
            <FinalResultsScreen authHeaders={authHeaders} onNeedsAuthReset={onNeedsAuthReset} />
          ) : null}
